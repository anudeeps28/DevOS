...
| Pipeline = OS-orchestrated **role sessions** per stage (the Bridge + Navigator/Shipwright/Lookout/Warden/Harbormaster), not one long lead-agent session; gated by default, per-project auto-advance toggle, interrupts always break through; role roster defined in the harness (SPEC §3.1) | Resolved with architect 2026-07-20 — automates the proven manual session-per-stage + handoff-artifact workflow; clean context per stage; orchestration in deterministic server code; keeps zero pipeline logic in DevOS. **SUPERSEDED 2026-07-31 by the two-session consolidation (next row)** | Yes — core model (superseded) |
| Pipeline consolidated to **two sessions per work item** — one build session (understand→plan→code→test→fix, the `/implement`/`/story` flow) + one fresh **report-only** adversarial review session; role names become roster-declared UI **phase labels**; phase markers written by skills at subagent boundaries; only BLOCK findings fail review (loop cap 3 → escalate); build drafts the PR, the Bridge opens it mechanically via the code-platform adapter; **PR = single default human gate** (optional per-project plan-gate toggle, interrupts always break through); context handled by per-wave checkpoint + respawn-at-~80%-of-window (never compact, ~2 resumes → escalate); roster also declares per-role **model + effort** (Opus 1M-context; builder medium, reviewer high) (SPEC §3.1 rev 2026-07-31) | Grill 2026-07-31 (9 forks, `grill-summary.md`) — real usage data: 5× cold-start overhead per item with no quality gain vs the proven single-session build flow; upstream task sizing (description + AC per task) supplies the clean-context benefit the relay provided; review independence is the one load-bearing separation and is kept; the Bridge is roster-driven so the change is mostly roster + `roles.ts` + rework-target data | Yes — core model |
