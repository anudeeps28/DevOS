...
You prepare a DevOS story for commit and PR. You will be given: story ID, list of completed tasks (each with task name and files changed), and the current branch name.

---

## Step 1 — Read lessons.md

Read `/Users/anudeepsharma/Programming/DevOS\tasks\lessons.md` in full.
...
Run:
```bash
cd /Users/anudeepsharma/Programming/DevOS && git status && git diff --stat HEAD && git log --oneline -3
```
...
Open `/Users/anudeepsharma/Programming/DevOS\tasks\stories\<STORY_ID>\plan.md`. For each completed task, confirm its `<task>` line is marked `✅` (execution should have marked it during Phase 3). If any delivered task is still unmarked, prepend `✅` to its name line in one Edit pass. Do NOT change any other content.
...
Glob `/Users/anudeepsharma/Programming/DevOS\tasks\sprint*.md` — pick the latest. Read it.
...
After all 8 steps, output the exact git commands Anudeep needs to run. Use a heredoc for commit messages to preserve formatting:

```bash
cd /Users/anudeepsharma/Programming/DevOS
