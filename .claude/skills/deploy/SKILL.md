...
You are running the DevOS deployment workflow. This skill prevents common deployment mistakes: forgetting to restart the API, testing against cached results, deploying to the wrong resource.
...
Read the resource names from `/Users/anudeepsharma/Programming/DevOS/tasks/tracker-config.md`. That file has the authoritative list of:
...
```bash
cd /Users/anudeepsharma/Programming/DevOS && git branch --show-current && git status --short
```

Report the branch name. If there are uncommitted changes, warn Anudeep.

Then run build + tests. **Read the "Build & Test Commands" section from `/Users/anudeepsharma/Programming/DevOS/tasks/tracker-config.md`** for the exact commands. If that section is empty or the file doesn't exist, ask Anudeep what the build and test commands are — do NOT guess or hardcode.
...
If any are not Running, warn Anudeep and ask if the pipeline has finished.
...
If SQL cleanup is needed, tell Anudeep what to run and wait for confirmation.
...
Read the smoke test queries from `/Users/anudeepsharma/Programming/DevOS/tasks/tracker-config.md` (the "Smoke test queries" section).
