...
You are the story execution orchestrator for DevOS.
...
Run these phases in strict order — Understand → **Goal Definition (1.5)** → Plan → Execute → Local Verify → Review → **e2e Goal Gate (3.7)** → PR. Each phase ends with a mandatory STOP checkpoint. **Do not advance to the next phase without Anudeep's explicit confirmation.**

---

## Before you start

Read `/Users/anudeepsharma/Programming/DevOS\tasks\lessons.md` now. It contains the git commit rules, Code Rabbit patterns, and the 3-attempt rule you must follow throughout.

Also run:
```bash
cd /Users/anudeepsharma/Programming/DevOS && git status && git branch --show-current
mkdir -p /Users/anudeepsharma/Programming/DevOS/tasks/stories/$ARGUMENTS
```
...
Glob `/Users/anudeepsharma/Programming/DevOS\tasks\sprint*.md` and pick the latest sprint file.
...
**Write the handoff contract:** Save the full brief to `/Users/anudeepsharma/Programming/DevOS/tasks/stories/$ARGUMENTS/brief.md` using the structure from the brief template. Include all 8 points.
...
Once Anudeep confirms Phase 1, define the **goal** before planning. This is what makes the story goal-driven: "done" is not "compiles + tasks ran" — it's this goal being met. See `rules/test-philosophy.md` (Level 3).

Work through this with Anudeep — it is a short, mandatory branch, not a full interview:
...
2. **Is there a machine oracle for this story's goal?** Yes → the gate is an automated check. No (needs a human ruling / subjective call) → the gate is a **structured human acceptance check** — the actual behavior is still shown (observability, below) and Anudeep signs off. Either way the story is gated; the gate is never skipped.
...
**Escape hatch:** for a change with zero runtime behavior (docs, comments, pure rename), Anudeep may say "skip gate — no runtime impact"; log it and proceed. This is the only way to skip the gate.
...
Do NOT proceed until Anudeep responds. The confirmed goal is the input to Phase 2 — the plan agent turns it into the test strategy + test/eval tasks.

---

## Phase 2 — Plan

Once Anudeep confirms Phase 1 (with or without corrections) and the Phase 1.5 goal:

If Anudeep gave corrections, append them to `/Users/anudeepsharma/Programming/DevOS/tasks/stories/$ARGUMENTS/brief.md` under the "Corrections from Anudeep" section.

Spawn a **`story-plan-agent`** (foreground) with the full Phase 1 brief as input, the **Phase 1.5 goal** (modality + concrete gate + acceptance-criteria-as-gate + observability), plus any corrections Anudeep gave. The plan agent turns the goal into the test-strategy block and the matching test/eval tasks.
...
- Verify the plan agent saved the full plan (XML + wave summary + rationale) to `/Users/anudeepsharma/Programming/DevOS/tasks/stories/$ARGUMENTS/plan.md` using the structure from the plan template. If it didn't, save it from the agent's output. This plan file — not `todo.md` — is what `/run-tasks` reads to resume execution if the session is interrupted; it lives in the always-local `tasks/stories/` workspace and works in every tracker mode.
- Verify the test strategy was saved to `/Users/anudeepsharma/Programming/DevOS/tasks/stories/$ARGUMENTS/test-strategy.md`. If it wasn't, extract the test strategy section from the plan output and save it there.
...
Do NOT proceed until Anudeep approves.

**Plan revision stall detection:** If Anudeep requests changes to the plan, re-spawn the plan agent with corrections. Track the number of issues/changes requested across revision iterations. If the issue count does not decrease between consecutive iterations (the plan is not converging), stop and say:
...
one item per pending `<task>` (across all waves), using the plan's task names — so Anudeep sees
...
3. **Tell Anudeep:** "Wave [n] was split due to file overlap. Revised wave plan: [show updated table]."
...
- `type="manual"` tasks: display the `<action>` as instructions for Anudeep. Do not spawn an agent. Treat as BLOCKED pending human confirmation.
...
**E2. Update the executor state handoff:** Write/update `/Users/anudeepsharma/Programming/DevOS/tasks/stories/$ARGUMENTS/executor-state.md` with the current progress table and wave log. Update after EVERY wave, not just at the end. This file is the source of truth for what's been done.
...
Do NOT start the next wave until Anudeep says "yes".
...
- **If any task FAILED or is BLOCKED**: STOP and show the full STOP 3 message above — auto-run pauses on failure. Anudeep must respond before continuing.
...
After all waves in Phase 3 are complete and Anudeep confirms, run `/local-test 2` to verify the full build, all tests, and end-to-end smoke test pass with the changes.
...
> Story ID: $ARGUMENTS
> Plan path: /Users/anudeepsharma/Programming/DevOS/tasks/stories/$ARGUMENTS/plan.md
> Scope: full

**Agent 2 — Acceptance Tester:** Spawn an **`acceptance-test-agent`** with:

> Story ID: $ARGUMENTS
> Test strategy path: /Users/anudeepsharma/Programming/DevOS/tasks/stories/$ARGUMENTS/test-strategy.md
> Plan path: /Users/anudeepsharma/Programming/DevOS/tasks/stories/$ARGUMENTS/plan.md
...
**Write the handoff contracts:**
- Save the evaluation report to `/Users/anudeepsharma/Programming/DevOS/tasks/stories/$ARGUMENTS/evaluation.md`
- Save the acceptance report to `/Users/anudeepsharma/Programming/DevOS/tasks/stories/$ARGUMENTS/acceptance.md`
- Save the architecture review to `/Users/anudeepsharma/Programming/DevOS/tasks/stories/$ARGUMENTS/architecture-review.md`
- Save the security review to `/Users/anudeepsharma/Programming/DevOS/tasks/stories/$ARGUMENTS/security-review.md`
...
Do NOT proceed until Anudeep confirms.
...
_(If Anudeep took the "skip gate — no runtime impact" escape hatch at Phase 1.5, note it and skip straight to Phase 4.)_
...
- **Structured human acceptance** (no machine oracle): show the ACTUAL behavior using the story's observability plan (API response, log, trace, screenshot — never a raw prod DB read), then ask Anudeep to sign off against each acceptance criterion.

**If the gate is green (or Anudeep accepts the human check):** the goal is met. Say:
...
A failed gate never triggers a blind re-run. Each iteration runs an explicit cycle, in-session with Anudeep at the gates (no background autonomous loop — only a slow deploy/build step, if any, may run in the background):
...
Once the goal is met (Phase 3.7 gate green or human-accepted) and Anudeep confirms:
...
Wait for Anudeep to run the git commands and confirm. Only then raise the PR using `gh pr create`.
...
- If Anudeep says "stop" at any point — stop immediately, summarize state, ask what to do next
