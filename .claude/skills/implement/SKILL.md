...
You are the implementation orchestrator for DevOS. You will build: **$ARGUMENTS**

Run these phases in order — **Understand (1)** → **Goal Definition (1.5)** → Plan → Execute → Local Verify → Evaluate → PR. Each phase ends with a STOP checkpoint. **Do not advance without Anudeep's confirmation.**

---

## Before you start

Read `/Users/anudeepsharma/Programming/DevOS/tasks/notes.md` if it exists — it contains conventions, known fixes, and decisions.
...
2. **Classify the remaining arguments:**
   ...
   - If the active tracker is `local`:
     ...
     - Plain text description → **no ID given.** Offer to register it first so the work lands in the local task registry:
       ...
       If Anudeep says **yes**: `bash trackers/active/create-issue.sh "<description>" "" ""` → capture the new numeric ID from the output and treat it as the task ID from here on (the render hook regenerates `todo.md`). If Anudeep says **skip**: proceed with the plain description and no registry entry — the zero-tracker escape hatch, still fully supported.
...
3. **Echo back** the parsed intent on one line, e.g. `Task: #42  |  Flags: --discuss --research` or `Task: "Build login flow" (Todoist)  |  Flags: --research`, so Anudeep can catch a typo before anything else runs.
...
**Write the handoff contract:** Save the full brief to `/Users/anudeepsharma/Programming/DevOS/tasks/stories/<id>/brief.md`. Include all points produced by the agent.
...
Ask Anudeep these 3 fixed questions in order, one at a time, waiting for an answer after each. If Anudeep has already answered any of them in the original `$ARGUMENTS`, **skip that question** and note it as "(already answered)":
...
Work through this with Anudeep — it is a short, mandatory step, not a full interview:
...
2. **Is there a machine oracle?** Yes → the gate is an automated check. No → the gate is a **structured human acceptance check** (the actual behavior is still shown via observability, Anudeep signs off).
...
**Escape hatch:** for a change with zero runtime behavior (docs, comments, pure rename), Anudeep may say "skip gate — no runtime impact"; log it and proceed.
...
> Project root: /Users/anudeepsharma/Programming/DevOS
>
> Pre-planning brief (from Phase 1):
> [full brief from the story-understand-agent]
>
> [If Anudeep gave corrections] Corrections:
...
- `type="manual"`: display instructions for Anudeep.
...
Do NOT start the next wave until Anudeep responds.

**If mode B (auto-run):**
- Show the wave result table so Anudeep can see progress in real-time.
- **If all tasks passed**: say "Wave [n] ✅ — continuing to Wave [n+1]..." and proceed immediately. Do NOT wait for confirmation.
- **If any task FAILED or BLOCKED**: STOP and show the full wave result — auto-run pauses on failure. Anudeep must respond before continuing.
...
> Plan path: /Users/anudeepsharma/Programming/DevOS/tasks/stories/<id>/plan.md
> Scope: quick (if < 5 files changed) or full (if >= 5 files changed)

**Agent 2 — Acceptance Tester:** Spawn an **`acceptance-test-agent`** with:

> Story ID: [issue ID or "implement/<branch-name>"]
> Test strategy path: /Users/anudeepsharma/Programming/DevOS/tasks/stories/<id>/test-strategy.md
> Plan path: /Users/anudeepsharma/Programming/DevOS/tasks/stories/<id>/plan.md
...
- If Anudeep says "stop" at any point → stop immediately
