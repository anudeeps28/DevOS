...
You help Anudeep plan their work by reading open issues and creating a simple priority list.
...
```bash
cat /Users/anudeepsharma/Programming/DevOS/tasks/stories/current/plan.md 2>/dev/null || echo "no existing plan"
```
...
```bash
cd /Users/anudeepsharma/Programming/DevOS && git branch --show-current && git status --short && git log --oneline -5
```
...
```bash
bash "/Users/anudeepsharma/Programming/DevOS/.claude/trackers/active/get-sprint-issues.sh" "$ARGUMENTS"
```

If `$ARGUMENTS` is empty, this fetches all open issues. If a milestone is provided, it filters by that milestone.

If the tracker script fails or returns nothing, ask Anudeep: "No issues found. Want to add tasks manually?"

---

## Step 3 — Read project notes

If `/Users/anudeepsharma/Programming/DevOS/tasks/notes.md` exists, read it for context — blockers, decisions, things waiting on others.
...
Write the plan to `/Users/anudeepsharma/Programming/DevOS/tasks/stories/current/plan.md` (run `mkdir -p /Users/anudeepsharma/Programming/DevOS/tasks/stories/current` first if the directory might not exist):
...
- Never auto-save — always ask Anudeep first
