...
Read `/Users/anudeepsharma/Programming/DevOS\tasks\lessons.md`. Note the "PR Comment Review Process" and "Patterns Code Rabbit Flags" sections.
...
If the branch shown does not match PR #$ARGUMENTS, tell Anudeep and stop.
...
```bash
bash "/Users/anudeepsharma/Programming/DevOS/.claude/code-platform/active/get-pr-review-threads.sh" $ARGUMENTS
```
...
- Say: **"3-attempt rule triggered for [file]:[line]. This thread has been addressed 3 times and Code Rabbit keeps re-raising it. Removing from this cycle — needs Anudeep's manual review."**
...
Do NOT proceed until Anudeep says "go". If corrections are given, apply them to the analyst's output and re-show the updated table, then wait again.

**Seed the live progress checklist for this loop.** Once Anudeep says "go", create (or, on later
loops, extend) a `TodoWrite` list with one item per approved thread — labelled fix or reply — so
Anudeep can watch each thread get resolved before anything is committed. Mark items `completed` as
...
> Apply these fix items to the DevOS codebase, then run full build and all tests.
>
> Fix items:
...
Show the diff:
```bash
cd /Users/anudeepsharma/Programming/DevOS && git diff
```
...
Do NOT commit until Anudeep says "commit".

### 2D. Commit and push

When Anudeep says "commit":

```bash
cd /Users/anudeepsharma/Programming/DevOS && git add -A && git commit -m "Fix Code Rabbit review comments" && git push
```
...
Do NOT post anything until Anudeep says "send". If edits are given, update the drafts and re-show, then wait again.
...
```bash
bash "/Users/anudeepsharma/Programming/DevOS/.claude/code-platform/active/reply-pr-thread.sh" $ARGUMENTS <THREAD_ID> "<REPLY_TEXT>"
```

```bash
bash "/Users/anudeepsharma/Programming/DevOS/.claude/code-platform/active/resolve-pr-thread.sh" $ARGUMENTS <THREAD_ID>
```
...
Do NOT proceed until Anudeep says "wait" or "check now". Do NOT skip this gate even if Code Rabbit appears to have already responded.

- If Anudeep says **"wait"**: run `sleep 600`, then go back to Phase 1. Increment loop counter.
- If Anudeep says **"check now"**: go back to Phase 1 immediately. Increment loop counter.
...
- Never skip a GATE — always wait for Anudeep's explicit word ("go", "commit", "send", "wait", "check now")
...
- If Anudeep says **"stop"** at any point → stop immediately, summarize current state (which threads are fixed/replied/pending) and wait
